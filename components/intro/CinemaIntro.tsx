...
export default function CinemaIntro() {
  ...
  const toggleSound = () => {
    ...
    if (next) {
      ...
      // NB: un play() rifiutato (autoplay) non deve far scattare il synth: solo l'evento "error" del file lo fa.
      a.play().catch(() => {});
    } else {
  ...
  useEffect(() => {
    ...
    const frame = () => {
      ...
      if (soundRef.current) {
        ...
        if (fileOk.current) {
          // usa SEMPRE il file reale finché è raggiungibile; il sintetizzatore è solo fallback d'errore
